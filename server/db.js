...
const authenticate = async({ username, password })=> {
  console.log('We are in authenticate Function in DBS');
  console.log('UserName: ' + username + ' Function in DBS');
  const SQL = `
    SELECT id, password
    FROM users
    WHERE username = $1
  ...
  await console.log(response.rows[0].id + ' Function in DBS');
...
const findUserByToken = async(token) => {
  let id;
  try {
    console.log('Hey righ below is the token');
    console.log(token);
